...
export const RegistroEquipos = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:5000/torneoVoley/equipos', formData, {
                headers: {
            ...
            setEquipo({ titulo: '', imagen: null, division: '', genero: '', jugadores: [''] });
        } catch (error) {
    ...
    return (
        <div className="formulario-equipo">
            ...
            <form className='form-group' onSubmit={handleSubmit}>
                <div>
                    <label>Título:</label>
                    <input type="text" name="titulo" value={equipo.titulo} onChange={handleInputChange} required />
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="division" value={equipo.division} onChange={handleInputChange} required>
                ...
                <div>
                    <label>Género:</label>
                    <select name="genero" value={equipo.genero} onChange={handleInputChange} required>
                        <option value="">Seleccione una opción</option>
                ...
                <div>
                    ...
                    {equipo.jugadores.map((jugador, index) => (
                        <div className="registro-equipos-jugador" key={index}>
                            <input className='jugadores-i'
                                type="text"
                                value={jugador}
                                onChange={(e) => handleJugadorChange(index, e.target.value)}
                                required
                            />
                            <button type="button" onClick={() => eliminarJugador(index)}>Eliminar</button>
                        </div>
                    ))}
            ...
            <ul className="registro-equipos-list">
                {equipos.map((equipo, index) => (
                    <li key={index}>
                        <h3>{equipo.titulo}</h3>
                        <img src={`http://localhost:5000/${equipo.imagen}`} alt={`${equipo.titulo} Logo`} />
